        ...
        while True:
            try:
                ...
                for tr in tqdm(tr_list):
                    ...
                    # 中文期刊
                    if Chinese(web) == True:
                        if len(web.find_element_by_xpath('//*[@id="ChDivSummaryMore"]').get_attribute('style'))==0:
                            web.find_element_by_xpath('//*[@id="ChDivSummaryMore"]').click()
                        # 抓取标题
                        ...
                        # 姓名
                        author_name = ''
                        name_h3 = web.find_element_by_xpath('//*[@id="authorpart"]')
                        spans = name_h3.find_elements_by_tag_name('span')
                        print(len(spans))
                        if len(spans) == 1:

                            name = web.find_element_by_xpath('//*[@id="authorpart"]/span').text
                            print(name)
                            author_name = name
                        else:
                            author_name = ''
                            for i in range(len(spans)):
                                name = web.find_element_by_xpath('//*[@id="authorpart"]/span['+str(i+1)+']').text
                                if i!=len(spans)-1:
                                    author_name += (name+';')
                                else:
                                    author_name += name
                        ...
                        # 单位
                        address_h3 = web.find_element_by_xpath('//*[@class="wx-tit"]/h3[2]')
                        tag_a = address_h3.find_elements_by_tag_name('a')
                        print(len(tag_a))
                        if len(tag_a)==1:
                            address = web.find_element_by_xpath('//*[@class="wx-tit"]/h3[2]/span/a').text
                            print(address)
                        else:
                            for i in range(len(tag_a)):
                                address = web.find_element_by_xpath('//*[@class="wx-tit"]/h3[2]/a['+str(i+1)+']').text
                                print(address)
                    ...
                    try:
                        ...
                        # 写入csv文件中
                        # fileHeader = ['title','abstract']
                        # content = [title,abstract,author_name]
                        # print(content)
                        # csvFile = open('./paperAbstract1.csv','a',newline='')
...
def Choose_year(web):
    ...
    while True:
        try:
            ...
            time.sleep(3)
            web.find_element_by_xpath('//*[@id="txtEndYear"]').send_keys(2022)
            time.sleep(3)
...
if __name__ == '__main__':
    if not os.path.exists('./Abstract'):
        os.mkdir('./Abstract')
    chrome_driver = r"E:\anaconda\anaconda3\Lib\site-packages\selenium\webdriver\chrome\chromedriver.exe"
    opt = Options()
    ...
    web = Chrome(options=opt,executable_path=chrome_driver)
    web.get(url)
    # while True:
    spider(web)
